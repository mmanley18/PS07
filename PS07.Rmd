...
> Because `age` has a distribution that most closely resembles a normal distribution, we will use this when building out binary classifier. From the ggplots, we see that `DebtRatio` and `MonthlyIncome` are heavily skewed to the right and thus might not be as strong predictors of `SeriousDlqin2yrs.`
